...
The `GRiwrm` function helps to create an object of class `GRiwrm`. It renames the columns of the `data.frame` and converts basin area into downstream sub-basin area for each node.

```{r}
griwrm <- GRiwrm(nodes, list(id = "gauge_id", down = "downstream_id", length = "distance_downstream"), convertAreas = TRUE)
griwrm
```
