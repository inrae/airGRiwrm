...
In this vignette, we are still using the study case of the vignette \#1
and \#2, and we are going to simulate a diversion at the node “54001” to
provide flow to the node “54029” in order to support low flows in the
Severn river.

This objective is to maintain a minimum flow equals to the 3th decile of
the flow at station “54029” without remaining less than the first decile
of the flow downstream the station “54001”.
...
> The diversion from “54001” is computed to maintain a minimum flow of
> 5.3 m3/s at the gauging station “54029”. The diversion is allowed as
> far as the flow at the gauging station “54001” is above 11.5 m3/s.
...
We produce below the same operations as in the vignette
...
The main difference here is that we need to provide a diverted flow and
a minimum remaining flow at station “54029”. As, the diverted flow will
be calculated during simulation, we provide a initial diverted flow
...
more `Controller`. The supervision time step will be here the same as
the simulation time step: 1 day. Each day, the decision is taken for the
...
On a Diversion node, the minimum remaining flow is provided with the
...
divert to the gauging station “54002”.

We need to enclose the logic function in a function factory providing
...
We declare the controller which defines where is the measurement `Y` ,
...
The node “54029” was initially an upstream node. As it receives water
...
To compare results with diversion and without diversion, we run the
...
Let’s plot the diverted flow, and compare the flow at stations 54029 and
...
![](V06_Modelling_regulated_diversion_files/figure-html/unnamed-chunk-13-1.png)
