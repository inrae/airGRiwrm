...
test_that("Error: Basin area too small", {
  expect_error(
    ConvertMeteoSD(
      matrix(rep(c(1, 1), dataNumRows), ncol = 2, byrow = TRUE),
      c(1, 1)
    ),
...
test_that("No upstream basin should return input", {
  ...
  expect_equal(
    ConvertMeteoSD(
      matrix(rep(c(1), dataNumRows), ncol = 1, byrow = TRUE),
      c(1)
    ),
    resultMatrix
  )
...
test_that("Same inputs should return same value", {
  ...
  expect_equal(
    ConvertMeteoSD(
      matrix(rep(c(1, 1, 1), dataNumRows), ncol = 3, byrow = TRUE),
      c(10, 1, 1)
    ),
    resultMatrix
  )
...
test_that("Downstream data should return 2", {
  ...
  expect_equal(
    ConvertMeteoSD(
      matrix(rep(c(1.5, 1), dataNumRows), ncol = 2, byrow = TRUE),
      c(2, 1)
    ),
    resultMatrix
  )
...
test_that("Downstream data should return 0", {
  expect_equal(
    ConvertMeteoSD(
      matrix(rep(c(1, 2), dataNumRows), ncol = 2, byrow = TRUE),
      c(2, 1)
    ),
    resultMatrix
  )
...
test_that("getUpstreamRunoffIds works", {
  ...
  expect_equal(
    sort(getUpstreamRunOffIds("Down", griwrm)),
    sort(nodes$id[nodes$id != "Inter"])
  )
})
