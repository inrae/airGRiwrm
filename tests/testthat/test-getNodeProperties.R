...
test_that("All nodes should have property: Diversion and DirectInjection == FALSE", {
  ...
  lapply(c("Diversion", "DirectInjection"), function(p) {
    prop_value <- sapply(griwrm$id, function(id) {
      getNodeProperties(id, griwrm)[[p]]
    })
    expect_equal(all(prop_value), FALSE)
  })
  prop_calibration <- sapply(griwrm$id, function(id) {
    getNodeProperties(id, griwrm)$calibration
  })
...
test_that("Ungauged station has 'calibration:Ungauged' property", {
  ...
  expect_equal(
    getNodeProperties("54029", CreateGRiwrm(nodes))$calibration,
    "Ungauged"
  )
})
