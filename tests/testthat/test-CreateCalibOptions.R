test_that("airGR::CreateCalibOptions should works", {
  ...
  expect_equal(
    CreateCalibOptions(RunModel_GR4J, FUN_CALIB = Calibration_Michel),
    CalibOptions
  )
  expect_equal(
    CreateCalibOptions("RunModel_GR4J", FUN_CALIB = Calibration_Michel),
...
test_that("`FixedParam` should be a character or a list", {
  ...
  expect_error(CreateCalibOptions(
    InputsModel,
    FixedParam = list(`54057` = "wrong")
  ))
})
...
test_that("List FixedParam should have correct ids", {
  expect_error(
    CreateCalibOptions(
      InputsModel,
      FixedParam = list(`54057` = FixedParam, `54002` = FixedParam)
    ),
    regexp = "54002"
  )
})
...
test_that("FixedParam works on various models", {
  ...
  expect_equal_map(
    lapply(CO, "[[", "FixedParam"),
    list(
      `54057` = as.logical(FixedParam[1:5]),
      `54032` = FixedParam,
      `54001` = FixedParam[2:9]
    )
  )
  CO2 <- CreateCalibOptions(
    InputsModel,
    FixedParam = list(`54057` = c(NA, 1:4), `*` = FixedParam)
  )
  expect_equal_map(
    lapply(CO2, "[[", "FixedParam"),
    list(`54057` = c(NA, 1:4), `54032` = FixedParam, `54001` = FixedParam[2:9])
  )
})
