...
test_that("RunModel_Diversion with zero diversion equals no diversion", {
  ...
  lapply(names(OM), function(id) {
    ...
    expect_equal(
      OM[[!!id]]$RunOptions$WarmUpQsim,
      OM_GriwrmInputs[[!!id]]$RunOptions$WarmUpQsim
    )
    expect_equal(
      OM[[!!id]]$RunOptions$WarmUpQsim_m3,
      OM_GriwrmInputs[[!!id]]$RunOptions$WarmUpQsim_m3
    )
  })
})
...
test_that("Huge diversion would result in Qsim_m3 == Qmin", {
  ...
  expect_equal(
    ...
    Qmin[RunOptions[[1]]$IndPeriod_Run] /
      g_div$area[g_div$id == "54029" & g_div$model != "Diversion"] /
      1E3
  )
})
...
test_that("Upstream node - equal Diversion should return same results", {
  ...
  Qinf[IndPeriod_WarmUp, 1] <- OM_GriwrmInputs[[
    "54095"
  ]]$RunOptions$WarmUpQsim_m3
  Qinf[IndPeriod_WarmUp, 2] <- -OM_GriwrmInputs[[
    "54095"
  ]]$RunOptions$WarmUpQsim_m3
  ...
  IM_54001_div <- UpdateQsimUpstream(
    IM_2ol[["54001"]],
    RO_2ol[["54001"]],
    OM_2ol
  )
  IM_54001 <- UpdateQsimUpstream(
    InputsModel[["54001"]],
    RunOptions[["54001"]],
    OM_GriwrmInputs
  )
...
test_that("RunModel should return water deficit (Qover_m3)", {
  ...
  expect_warning(
    e <- setupRunModel(griwrm = g, runRunModel = TRUE, Qinf = Qinf)
  )
  for (x in ls(e)) {
    assign(x, get(x, e))
  }
