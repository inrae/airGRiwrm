...
test_that("Ungauged node with upstream node with diversion should work", {
  ...
  ParamRef[["P"]] <- 1
  OM <- RunModel(InputsModel, RunOptions = RunOptions, Param = ParamRef)
  ...
  expect_true(
    !any(
      g_reduced$id == "P" &
        !is.na(g_reduced$model) &
        g_reduced$model == "Diversion"
    )
  )
  expect_true(any(
    g_reduced$id == "54001" &
  ...
  IC <- CreateInputsCrit(
    ...
    Obs = Qobs[
      IndPeriod_Run,
      np$id[np$RunOff & np$calibration == "Gauged"],
      drop = FALSE
    ],
  )
...
test_that("Donor node with diversion should work", {
  ...
  expect_equal(
    OC_ref$`54032`$CritFinal,
    OutputsCalib$`54032`$CritFinal,
    tolerance = 1E-3
  )
})
...
test_that("Cemaneige with hysteresis works", {
  ...
  expect_equal(sapply(Param, length), c("54057" = 9, "54001" = 8, "54032" = 9))
})
...
test_that("Ungauged upstream node with upstream donor works", {
  ...
  expect_equal(
    OutputsCalib$`54095`$ParamFinalR[1:3],
